...
export default function ApplyItem(props) {
  ...
  // 展示客户信息
  const customizedInfo = showCustomerInfo(data);
  const customerInfo = _.isEmpty(customizedInfo) ? `客户：${data.custName || '无'}(${data.custNumber || '无'})` : customizedInfo;
  ...
  return (
    <div className={applyItemCls} onClick={() => onClick(data, index)}>
      ...
      <div className={thirdLineCls}>
        ...
        <div className={styles.customer} title={customerInfo}>{customerInfo}</div>
      </div>
    </div>
  );
}
...
ApplyItem.propTypes = {
  ...
  // 其返回值为字符串
  showCustomerInfo: PropTypes.func,
};

ApplyItem.defaultProps = {
  typeName: '',
  active: false,
  showCustomerInfo: _.noop,
};
