 ...
 * @Last Modified time: 2018-09-18 15:59:32
...
export default class AdvisorSpace extends PureComponent {
  ...
  getRightDetail() {
    ...
    if (!_.isEmpty(applictionList.applicationBaseInfoList)) {
      // 表示左侧列表获取完毕
      // 因此此时获取Detail
      const { pageNum, pageSize } = applictionList.page;
      let item = applictionList.applicationBaseInfoList[0];
      let itemIndex = _.findIndex(applictionList.applicationBaseInfoList, o => o.id.toString() === currentId);
      if (!_.isEmpty(currentId) && itemIndex > -1) {
        // 此时url中存在currentId
        item = _.filter(applictionList.applicationBaseInfoList, o => String(o.id) === currentId)[0];
      } else {
        // 不存在currentId
        replace({
          pathname,
